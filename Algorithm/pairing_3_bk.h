...
#endif
#if 1
    aes state;//2021update
    Big count;
#define AES_MAX 256
    struct AES_Big_CIPHER
    {
        int x_len;
        char x[AES_MAX];
    };
    struct AES_G1_CIPHER
    {
        char data[AES_MAX];
    };
    struct AES_G2_CIPHER
    {
        char data[AES_MAX];
    };
    struct AES_GT_CIPHER
    {
        char data[AES_MAX];
    };
#endif
...
// creates a smaller table
    ...
    //aes update2021
    int aes_start(char *key,char *iv);
    int aes_en(Big &x, AES_Big_CIPHER &y);
#if 0
    int aes_en(G1 &x,AES_CIPHER &y);
    int aes_en(G2 &x,AES_CIPHER &y);
    int aes_en(GT &x,AES_CIPHER &y);
    int aes_de(AES_CIPHER &x,Big &y);
    int aes_de(AES_CIPHER &x,G1 &y);
    int aes_de(AES_CIPHER &x,G2 &y);
    int aes_de(AES_CIPHER &x,GT &y);
    int aes_end();
#endif
