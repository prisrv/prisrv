...
#define AES_NK 16
int PFC::aes_start(char *key,char *iv)
{
    count=0;
    return aes_init(&state,MR_ECB,AES_NK,key,iv);

}
int PFC::aes_en(Big &x,AES_Big_CIPHER &y)
{
    big b=x.getbig();
    for(int i=0;i<AES_MAX;i++)
        y.x[i]=0;
    y.x_len=big_to_bytes(0,b,y.x,false);
    char m[16];
   // for(int i=0;i<)
    big_to_bytes(16,count.fn,m,false);
